...
def load_dataset(lookback):
    data = pd.read_csv('globa_superstore.csv', encoding='unicode_escape')
...
if __name__ == "__main__":
    lookback = 3
    timeseries, x_train, y_train, x_test, y_test = load_dataset(lookback)
    model = train(x_train, y_train, x_test, y_test)
    create_plot(model, timeseries, x_train, x_test, lookback)